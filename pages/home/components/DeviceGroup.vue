	...
	export default {
		...
		created() {
			...
			this.SelectArr.map(item => {
				item.specialDevice = specialDeviceArr.includes(item.model)
			})
			this.$forceUpdate()
			this.serviceid_list = []
			const isFirstGroup = uni.getSystemInfoSync('isFirstGroup')
			if (!isFirstGroup) {
		...
		methods: {
			...
			handleGroupDeviceDetail(deviceList, services_list) {
				this.canClick = false
				uni.navigateTo({
					url: '/pages/device/DeviceGroup/DeviceGroup?list=' + JSON.stringify(deviceList) +
