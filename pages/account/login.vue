<template>
	<PageLayout :navigator-content="tempCommunityName" :pageHeight="2">
		<view class="login-page">
			...
			<view class="login-page__items">
				<view class="login-page__items__item" v-for="(titem,index) in platforms" :key="index"
					v-show="!titem.isHide" @click="clickPlatform(titem.scope)">
					<image v-if="index != 0" mode="aspectFill" :src="titem.icon">
					</image>


					<uni-icons v-else type="email" style="margin-right: 15rpx;" size="60rpx"></uni-icons>
					<!-- <u-icon v-else type="email" style="margin-right: 15rpx;" size="60rpx"></u-icon> -->

					<view class="Title_box" style="font-size: 42rpx;">{{titem.name}}</view>
				</view>
			</view>

			<view class="login-page__forgot-password Title_box"
			...
			<view class="login-page__agreement">
				<Agreement :locale="commonStore.locale" :check-agreement.sync="checkAgreement"
					:temp-community-id="tempCommunityId" />
			</view>
			<view class="login-page__create"
				...
				<text class="Title_box">{{ $t('login_page.create_new_account') }}</text>
			</view>

			<u-popup ref="newUser" type="center" :mask-click="false">
				<view class="set_passward_box">
					<input @input="newPasswardInput" :value="newPassward"
						:placeholder="$t('password_page.new_password_empty_hint')" class="myinput" />
					<view class="change-password" @click="setNewPassward">{{$t('save')}}</view>
				</view>
			</u-popup>
			<!-- <uni-popup ref="newUser" type="center" :is-mask-click="false">
				<view class="set_passward_box">
					<input @input="newPasswardInput" :value="newPassward"
						:placeholder="$t('password_page.new_password_empty_hint')" class="myinput" />
					<view class="change-password" @click="setNewPassward">{{$t('save')}}</view>
	...
	export default {
		...
		data() {
			return {
				...
				platforms: [{
					...
					// {
					// 	name: 'facebook',
					// 	icon: require('/static/images/settings/facebook.png'),

					// },
					// {
					// 	name: 'twitter',
					// 	icon: require('/static/images/settings/twitter.png')
					// },
					{
						name: 'Amazon',
						scope: 'amazon',
		...
		methods: {
			...
			newPasswardInput(e) {
				...
				this.newPassward = e.target.value
			},
			async setNewPassward() {
				let that = this
				if (this.newPassward.length < 5) {
					console.log(this.newPassward.length)
					let content = this.$t('password_page.password_invalid_hint')
					let type = 'error'
					this.$toast({
						content,
						type
				...
				const params = {
					old_passwd: '123456',
					new_passwd: this.newPassward
				}

				let content = this.$t('password_page.save_password')
				let type = 'success'
				this.$loading()
				try {
					const {
					...
					if (code === 0) {
						...
						// that.$refs.newUser.close()
						return that.toHomePage()
					}

					// 异常处理
					const codeMap = {
						1017: this.$t('register_page.verfication_code_incorrect'), // 验证码不对
						1018: this.$t('password_page.old_password_invalid_hint') // 旧密码不对
					}
			...
			clickPlatform(platform) {
				...
				if (platform === "email") {
					uni.navigateTo({
						url: `/pages/account/LoginDetail?communityId=${this.tempCommunityId}&communityName=${this.tempCommunityName}`,
						success() {
			...
			async thirdLogin(obj, platform) {
				...
				try {
					...
					if (code === 1012) return
					if (code === 1020) {
						if (plus.os.name === 'Android' && platform === "google") {
							this.platforms[1].isHide = true
						}
						// return
						// this.thirdLogin(obj, platform)
						// this.$toast({ content: this.$t('login_page.email_unregistered'), type: 'error' })
						this.$navigateTo(
							`/pages/account/thirdParty?platform=${changeKey[platform]}&id=${data.id}&name=${nick_name}&email=${obj.email}&openId=${obj.openId || obj.id || obj.user_id}&communityName=${this.tempCommunityName}&communityId=${this.tempCommunityId}`
						)
						return
					}
					// 异常处理
